...
parser.add_argument('--config-file', nargs='+', type=str,
                    default=os.path.join('sga_conf.yaml'),
                    help='Config YAML files. The script deduplicates the configs, but does not check them.')

parser.add_argument('--kwargs', type=json.loads, default='{}',
                    help='Will overwrite the loaded config')
...
def main(args: argparse.Namespace):
    configs, run = build_configs_and_run(
        [args.config_file], 'examples/Graph_Adversarial_Learning/Targeted/Poisoning/PyTorch/sga_experiment.py', args.kwargs)
