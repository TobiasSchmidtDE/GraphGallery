...
def run(data_dir: str, dataset: str, attack: str, attack_params: Dict[str, Any], nodes: str, seed: int,
    ...
    models_and_hyperparams = storage.find_models(
        model_storage_type, model_params)

    model_params["label"] = surrogate_model_label
    surrogate_models_and_hyperparams = storage.find_models(
        surrogate_model_storage_type, model_params)

    assert len(
        surrogate_models_and_hyperparams) == 1, "Not exactly one surrogate model"
    assert len(models_and_hyperparams) == 1, "Not exactly one victim model"

    surrogate_model = surrogate_models_and_hyperparams[-1][0]

    victim_model, hyperparams = models_and_hyperparams[-1]
    ...
    if nodes is None:
        victim_model.eval()
        logits = victim_model(data=torch.tensor(graph.node_attr).to(device),
                              adj=SparseTensor.from_scipy(graph.adj_matrix).to(device))
        logits = torch.nn.functional.softmax(logits, dim=-1)[splits.test_nodes]

        acc = (logits.argmax(1) ==
               torch.tensor(graph.node_label).to(device)[splits.test_nodes]).float().mean().item()
        ...
        (max_confidence_nodes_idx,
         ...
         rand_nodes_idx) = sample_attack_nodes(logits.cpu().detach().numpy(),
                                               graph.node_label[splits.test_nodes],
                                               splits.test_nodes,
                                               graph.adj_matrix,
                                               int(nodes_topk / 4),  min_node_degree)
        tmp_nodes = np.concatenate((max_confidence_nodes_idx,
                                    min_confidence_nodes_idx,
                                    rand_nodes_idx))
    print(tmp_nodes)
    ...
    for target in tmp_nodes:
        ...
        victim_model.eval()
        initial_logits = victim_model(data=torch.tensor(graph.node_attr).to(device),
                                      adj=SparseTensor.from_scipy(graph.adj_matrix).to(device))
        initial_logits = torch.nn.functional.softmax(initial_logits[target],
                                                     dim=-1).cpu().detach().numpy()
        initial_classification_statistics = classification_statistics(initial_logits,
                                                                      target_label)

        ################### Attacker model ############################
        attacker = gg.attack.targeted.SGA_RGNN_AT_SCALE(graph.copy(),
                                                        seed=seed,
                                                        device=device).process(surrogate_model)
        attacker.attack(target)
        attacker.show()
        ################### Evasion Evaluation ############################
        victim_model.eval()
        if hasattr(victim_model, 'release_cache'):
            victim_model.release_cache()
        if hasattr(victim_model, "deactivate_caching"):
            victim_model.deactivate_caching()

        graph.adj_matrix

        evasion_logits = victim_model(data=torch.tensor(attacker.g.node_attr).to(device),
                                      adj=SparseTensor.from_scipy(attacker.g.adj_matrix).to(device))
        evasion_logits = torch.nn.functional.softmax(evasion_logits[target],
                                                     dim=-1).cpu().detach().numpy()
        ...
        victim_poisoned_model = deepcopy(victim_model).to(device)
        for module in victim_poisoned_model.modules():
            if hasattr(module, 'reset_parameters'):
                module.reset_parameters()

        labels = torch.tensor(graph.node_label).to(device).to(torch.long)
        attr_adversary = torch.tensor(attacker.g.node_attr).to(device)
        adj_adversary = SparseTensor.from_scipy(
            attacker.g.adj_matrix).to(device)

        if hasattr(victim_poisoned_model, 'fit'):
            if hasattr(victim_poisoned_model, 'ppr_cache_params'):  # TODO: How to do this porperly?
                victim_poisoned_model.ppr_cache_params = None

            _ = victim_poisoned_model.fit(attr_adversary,
                                          attr_adversary,
                                          labels=labels,
                                          idx_train=splits.train_nodes,
                                          idx_val=splits.val_nodes,
                                          dataset=dataset,
                                          make_undirected=make_undirected,
                                          **hyperparams['train_params'])
        else:
            _ = train(model=victim_poisoned_model, attr=attr_adversary, adj=adj_adversary,
                      labels=labels, idx_train=splits.train_nodes, idx_val=splits.val_nodes, **hyperparams['train_params']
                      )

        victim_poisoned_model.eval()
        poisoned_logits = victim_poisoned_model(data=attr_adversary,
                                                adj=adj_adversary)
        poisoned_logits = torch.nn.functional.softmax(poisoned_logits[target],
                                                      dim=-1).cpu().detach().numpy()
